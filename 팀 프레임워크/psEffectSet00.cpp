...
void psEffectSet00::EffectSet02(string psName)
{
	...
	_boardEMT = NULL;
	_boardEMT = new dxBoardEmitter;
	_boardEMT->init("ParticleResources/Texture/GlassShatterRad.png", 2, 0.5f, 2);
	//_pointEMT->InitConstSize(2.4f);
	...
	_planeEMT->init("ParticleResources/Texture/GlassShatterRad.png", 3, 0.5f, 3);

	_planeEMT->InitRandomSize(0.5f, 0.6f);
	_planeEMT->InitRandomSizeEnd(2.5f, 3.5f);
	...
	_planeEMT->InitRandomAlpha(0.1f, 0.1f);
	_planeEMT->addAlphaGraph(0.8f, 0.7f, 0.7f);
...
void psEffectSet00::EffectSet04(string psName)
{
	...
	_boardEMT->InitRandomLifeTime(1.2f, 1.5f);

	_boardEMT->InitRandomSize(0.2f, 0.4f);
	_boardEMT->InitRandomSizeEnd(3.2f, 6.2f);

	_boardEMT->SetPositionType(PTC_SPHERE);
	_boardEMT->InitDirSphere(1.0f, 0.0f, 1.0f);
	_boardEMT->InitRandomPosSphereRadius(1.0f, 2.0f);
	_boardEMT->InitRandomPosExplosionVel(1.5f, 3.0f);

	_boardEMT->InitRandomVelocityY(0.1f, 0.2f);
