...
namespace dx {
	...
	float transform::nowAngleH = 0.0f;					//수평앵글
	float transform::nowAngleV = 0.0f;					//수직앵글
	float transform::maxAngleV = 90.0f;					//수직 최대 앵글
	float transform::minAngleV = -90.0f;				//수직 최저 앵글
	float transform::sensitivityH = 1.0f;				//가로 민감도
	float transform::sensitivityV = 1.0f;				//세로 민감도 ( 이값이 음수면 Invert Mouse )
	D3DXVECTOR3 transform::nowVelocity = D3DXVECTOR3(0, 0, 0);	//현재 방향과 속도를 가진 벡터

	float transform::accelate = 30.0f;					//초당 이동 증가값
	float transform::nowSpeed = 3.0f;					//현재 속도
	float transform::maxSpeed = 10.0f;					//최고 속도 
	...
	void transform::DefaultControl(float _timeDelta, bool isKMW)
	{
		m_fNowAngleH = nowAngleH;
		m_fNowAngleV = nowAngleV;
		m_fMaxAngleV = maxAngleV;
		m_fMinAngleV = minAngleV;
		m_fSensitivityH = sensitivityH;
		m_fSensitivityV = sensitivityV;
		m_vNowVelocity = nowVelocity;
	...
	//디폴트 컨트롤 을해준다. - 새로 추가
	void transform::DefaultControl(float _timeDelta, float tmep)
	{
