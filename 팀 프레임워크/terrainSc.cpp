...
HRESULT terrainSc::init(char * heightMapName, char * tile_0, char * tileSplat, float cellSize, float heightScale, int smoothLevel, int tileNum)
{
	//스케일값 대입
	...
	//스플랫처 텍스쳐 로딩
	_texSlat = tileSplat != NULL ? RM_TEXTURE->getResource(tileSplat) : NULL;
...
HRESULT terrainSc::initMap(int heightNumber, int sizeX, int sizeZ, float cellSize, float heightScale, int smoothLevel, int tileNum)
{
	if (heightNumber == 1) heightMAPText = "Resource/Maptool/Textures/RAW/MyHeight0.bmp";
	if (heightNumber == 2) heightMAPText = "Resource/Maptool/Textures/RAW/Sheep_Height.bmp";
	if (heightNumber == 3) heightMAPText = "Resource/Maptool/Textures/RAW/MyHeight128.bmp";
	...
	//높이맵 불러온다
	_heightMap = RM_TEXTURE->getResource(heightMAPText);
	//불러오는데 실패하면 바로 종료
	if (_heightMap == NULL) return E_FAIL;
	...
	//가로세로 정점 수를 구한다.
	_verNumX = sd.Width + 1;	//가로 정점갯수는 높이 맵에 가로 해상도 + 1 과 같다. (이유는 쿼드트리쓰려면 정점갯수가 2의N승 + 1 이여야 하기 때문에)
	_verNumZ = sd.Width + 1;	//세로 정점잿수는 높이 맵에 세로 해상도 + 1 과 같다. (이유는 쿼드트리쓰려면 정점갯수가 2의N승 + 1 이여야 하기 때문에)
	_totalVerNum = _verNumX * _verNumZ;	//총 정점 갯수
...
void terrainSc::terrainUpdate(int number)
{
	if (number != 0)
	{
		if (number == 1)
		{
			_texDiffuse = RM_TEXTURE->getResource("Resource/Maptool/Textures/diffuseMap/map_diffuse00.png");
		}

		if (number == 2)
		{
			_texDiffuse = RM_TEXTURE->getResource("Resource/Maptool/Textures/diffuseMap/map_diffuse01.png");
		}
	}
}

void terrainSc::terrainUpdate2(int detailNumber)
{
	if (detailNumber != 0)
	{
		if (detailNumber == 1)
		{
			_texDetail = RM_TEXTURE->getResource("Resource/Maptool/Textures/detailMap/map_dtail00.png");
		}

		if (detailNumber == 2)
		{
			_texDetail = RM_TEXTURE->getResource("Resource/Maptool/Textures/detailMap/map_dtail06.png");
		}
	}
}

void terrainSc::release(void)
{
	...
	SAFE_RELEASE(_texDiffuse);
	SAFE_RELEASE(_texDetail);
}

void terrainSc::render(camera * cam, lightDirection * directionLight)
{
	//셰이더 렌더
	if (_texTile_0 != NULL)
	{
		...
		//셰이더이펙트 텍스쳐 세팅
		_terrainEffect->SetTexture("Terrain3_Tex", _texTile_0);
...
bool terrainSc::isIntersectRay(D3DXVECTOR3 * pOut, LPRay ray)
{
...
float terrainSc::getHeight(float x, float z)
{
...
bool terrainSc::getSlant(D3DXVECTOR3 * pOut, float gravityPower, float x, float z)
{
	//터레인 범위을 넘어가면 0.0 값을 리턴한다
	if (x < _terrainStartX || x > _terrainEndX ||
		z > _terrainStartZ || z < _terrainEndZ)
...
void terrainSc::setHeight(float x, float z, float delta)
{
	//터레인 범위을 넘어가면 0.0 값을 리턴한다
	if (x < _terrainStartX || x > _terrainEndX ||
		z > _terrainStartZ || z < _terrainEndZ)
	...
	_terrainVertices[idxZ * _verNumX + idxX].pos.y += delta;
	_terrainVertices[idxZ * _verNumX + idxX + 1].pos.y += delta;
	_terrainVertices[(idxZ + 1) * _verNumX + idxX].pos.y += delta;
	_terrainVertices[(idxZ + 1) * _verNumX + idxX + 1].pos.y += delta;
...
void terrainSc::render(int diffuseNumber, int detailNumber)
{
	...
	_device->SetTextureStageState(1, D3DTSS_COLOROP, D3DTOP_MODULATE2X);
	...
	if (diffuseNumber != 0)
	{
		_device->SetTexture(0, _texDiffuse);
	}

	if (detailNumber != 0)
	{
		_device->SetTexture(1, _texDetail);
	}
...
HRESULT terrainSc::createTerrain(int smooth, int tileNum)
{
	// 정점 위치 구한다.
...
void terrainSc::smoothTerrain(int passed)
{
	if (passed <= 0) return;
