...
void particleTest13::update()
{
	...
	{
		SKM->findSK("매직슈터")->setSkillPosTrans(_player);
		SKM->findSK("매직슈터")->setSkillDirTrans(_player);
		SKM->findSK("매직슈터")->setOneTargetTrans(_target);
	...
	{
		SKM->findSK("힐")->setSkillPosTrans(_player);
		SKM->findSK("힐")->setSkillDirTrans(_player);
		SKM->findSK("힐")->setOneTargetTrans(_target);
		SKM->findSK("힐")->Start();
