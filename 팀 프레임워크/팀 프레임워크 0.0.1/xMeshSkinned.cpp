...
void xMeshSkinned::update()
{
	...
	{
		//남은 크로스페이드 시간 뺀다
		this->_fLeftCrossFadeTime -= _timeDelta;
	...
	if (pBone->pMeshContainer)
	{
		...
		if (pBoneMesh->BufBoneCombos != NULL)
		{
			...
			for (DWORD i = 0; i < pBoneMesh->NumAttributesGroup; i++)
			{
				...
				//위에서 세팅됭 작업행렬을 Effect 팔래스에 적용한다.
				_pSkinnedEffect->SetMatrixArray(
					"amPalette", _pmWorkingPalette, pBoneMesh->NumPaletteEntries);
...
void xMeshSkinned::RenderChildBoneName(camera * pCam, BONE * pBone, D3DXVECTOR3 * parentPos)
{
	...
	if (pBone == _pRootbone)
	{
		...
		//본위치에 Gizmo
		GIZMOMANAGER->WireSphere(bonePos, 0.2f, color);
	}

	//본위치에 Gizmo
	GIZMOMANAGER->WireSphere(bonePos, 0.1f, color);

	//부모위치에서 내위치의 선
	if (parentPos != NULL) GIZMOMANAGER->Line(bonePos, *parentPos, color);
...
void xMeshSkinned::SetAnimation(LPD3DXANIMATIONSET animSet)
{
	...
	if (this->_fCrossFadeTime > 0.0f)
	{
		...
		//지금세팅되는 Animation 을 0 번Track 으로 세팅
