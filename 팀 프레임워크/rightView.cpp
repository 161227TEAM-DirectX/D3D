...
HRESULT rightView::init(void)
{
	...
	_OBJportalButton.uiNumber = OBSTAGEDOWN::ONEOFF;
	//환경하단 메뉴 
...
void rightView::update(void)
{
	...
	if (_TOPbutton[0].uiNumber == UISTAGE::UION)
	{
		if (_OBJbutton.uiNumber == OBSTAGE::STORMTOWN1 ||
			...
			_OBJbutton.uiNumber == OBSTAGE::PORTAL     &&  _OBJGO.uiNumber == UISTAGE::UION)
		{
			...
			if (_OBJportalButton.uiNumber == OBSTAGEDOWN::THREEOFF) _objectClass->objectUp(mouse, 188, 192);
		}
	}
}
...
void rightView::buttonObMiddle()
{
	if (_TOPbutton[0].uiNumber == UISTAGE::UION)
	{
		...
		if (_OBJbutton.uiNumber == OBSTAGE::PORTAL)
		{
			_OBJbutton.tex = RM_TEXTURE->getResource("Resource/Maptool/maptoolui/오브젝트중단.png");
			SPRITEMANAGER->renderRectTexture(_OBJbutton.tex, &_OBJbutton.rc1, &_OBJbutton.rc2, 0, 172, 244, 215, rightViewPort.X + (DWORD)73, rightViewPort.Y + (DWORD)95);

			if (_OBJGO.uiNumber == UISTAGE::UIOFF)
			{
				_OBJportalButton.rc2 = { (LONG)(rightViewPort.X + 70),(LONG)(rightViewPort.Y + 200),(LONG)(rightViewPort.X + 328),(LONG)(rightViewPort.Y + 234) };

				if (_OBJportalButton.uiNumber == OBSTAGEDOWN::ONEOFF)
				{
					_OBJportalButton.tex = RM_TEXTURE->getResource("Resource/Maptool/maptoolui/포탈메뉴.png");
					SPRITEMANAGER->renderRectTexture(_OBJportalButton.tex, &_OBJportalButton.rc1, &_OBJportalButton.rc2, 0, 0, 258, 34, rightViewPort.X + (DWORD)70, rightViewPort.Y + (DWORD)200);
				}
				else if (_OBJportalButton.uiNumber == OBSTAGEDOWN::TWOOFF)
				{
					_OBJportalButton.tex = RM_TEXTURE->getResource("Resource/Maptool/maptoolui/포탈메뉴.png");
					SPRITEMANAGER->renderRectTexture(_OBJportalButton.tex, &_OBJportalButton.rc1, &_OBJportalButton.rc2, 0, 34, 258, 68, rightViewPort.X + (DWORD)70, rightViewPort.Y + (DWORD)200);
				}
			}
			...
			if (_OBJGO.uiNumber == UISTAGE::UION)
			{
				...
				if (_OBJportalButton.uiNumber == OBSTAGEDOWN::THREEOFF)
				{
					_objectClass->portal();
				}
			}
...
void rightView::buttonObUpdate()
{
		...
		if (_OBJGO.uiNumber == UISTAGE::UIOFF)
		{
			...
			if (PtInRect(&_OBJbutton.rc3, GetMousePos()))
			{
				if (KEYMANAGER->isOnceKeyDown(VK_LBUTTON))
				{
					...
					{
						_OBJbutton.uiNumber = OBSTAGE::PORTAL;
					}
					else
					{
						_OBJbutton.uiNumber += 1;
	...
	if (_TOPbutton[0].uiNumber == UISTAGE::UION)
	{
		if (_OBJbutton.uiNumber == OBSTAGE::PORTAL)
		{
			if (_OBJGO.uiNumber == UISTAGE::UIOFF)
			{
				if (PtInRect(&_OBJportalButton.rc2, GetMousePos()))
				{
					_OBJportalButton.uiNumber = OBSTAGEDOWN::TWOOFF;
					//누르면 번호 넘겨주는거 해야됨
					if (KEYMANAGER->isOnceKeyDown(VK_LBUTTON))
					{
						_OBJGO.uiNumber = UISTAGE::UION;
						_OBJportalButton.uiNumber = OBSTAGEDOWN::THREEOFF;
					}
				}
				else
				{
...
void rightView::buttonENVMiddle()
{
	...
	if (_TOPbutton[2].uiNumber == UISTAGE::UION)
	{
		...
		if (_ENVbutton.uiNumber == ENVSTAGE::MONSTER)
		{
			...
			//렌더
			_objectClass->monster(numberMonster);
		}
	}
}
