...
HRESULT objectClass::init(void)
{
	...
	{
		_monsterObject.push_back(new baseObject);
		_monsterObject[i]->_transform->SetLocalPosition(0.0f, -0.2f, 0.0f);
		_monsterObject[i]->setActive(true);
	}
	...
	//포탈--------------------------------------------------------------------
	_object[188]->setMesh(XMESH_MANAGER->GetXmeshStatic("portal1"));
	_object[189]->setMesh(XMESH_MANAGER->GetXmeshStatic("portal2"));
	_object[190]->setMesh(XMESH_MANAGER->GetXmeshStatic("portal3"));
	_object[191]->setMesh(XMESH_MANAGER->GetXmeshStatic("portal4"));

	//몬스터----------------------------------------------------------------
	_monsterObject[0]->setMesh(XMESH_MANAGER->GetXmeshSkinned("골렘"));
	_monsterObject[1]->setMesh(XMESH_MANAGER->GetXmeshSkinned("개"));
	_monsterObject[2]->setMesh(XMESH_MANAGER->GetXmeshSkinned("멧돼지"));
	_monsterObject[3]->setMesh(XMESH_MANAGER->GetXmeshSkinned("나무짐승"));
	_monsterObject[4]->setMesh(XMESH_MANAGER->GetXmeshSkinned("브루탈루스"));
	_monsterObject[5]->setMesh(XMESH_MANAGER->GetXmeshSkinned("히드라"));
	_monsterObject[6]->setMesh(XMESH_MANAGER->GetXmeshSkinned("새"));
	_monsterObject[7]->setMesh(XMESH_MANAGER->GetXmeshSkinned("악어"));
	_monsterObject[8]->setMesh(XMESH_MANAGER->GetXmeshSkinned("곰"));
	_monsterObject[9]->setMesh(XMESH_MANAGER->GetXmeshSkinned("둠가드"));
	_monsterObject[10]->setMesh(XMESH_MANAGER->GetXmeshSkinned("하피"));
	_monsterObject[11]->setMesh(XMESH_MANAGER->GetXmeshSkinned("랩터"));
	_monsterObject[12]->setMesh(XMESH_MANAGER->GetXmeshSkinned("전갈"));
	_monsterObject[13]->setMesh(XMESH_MANAGER->GetXmeshSkinned("도마뱀"));
...
void objectClass::update(void)
{
	...
	for (int i = 0; i < MONSNUM; i++)
	{
		_monsterObject[i]->_transform->RotateSelf(0.0f, 0.05f, 0.0f);
	}
}
...
void objectClass::portal()
{
	_build[188].rc2 = { (LONG)(rightViewPort.X + 80),(LONG)(rightViewPort.Y + 230),(LONG)(rightViewPort.X + 120),(LONG)(rightViewPort.Y + 270) };
	_build[188].tex = RM_TEXTURE->getResource("Resource/Maptool/maptoolui/1.png");
	SPRITEMANAGER->renderRectTexture(_build[188].tex, &_build[188].rc1, &_build[188].rc2, 0, 0, 64, 64, rightViewPort.X + (DWORD)80, rightViewPort.Y + (DWORD)230);

	_build[189].rc2 = { (LONG)(rightViewPort.X + 130),(LONG)(rightViewPort.Y + 230),(LONG)(rightViewPort.X + 170),(LONG)(rightViewPort.Y + 270) };
	_build[189].tex = RM_TEXTURE->getResource("Resource/Maptool/maptoolui/2.png");
	SPRITEMANAGER->renderRectTexture(_build[189].tex, &_build[189].rc1, &_build[189].rc2, 0, 0, 64, 64, rightViewPort.X + (DWORD)130, rightViewPort.Y + (DWORD)230);

	_build[190].rc2 = { (LONG)(rightViewPort.X + 180),(LONG)(rightViewPort.Y + 230),(LONG)(rightViewPort.X + 220),(LONG)(rightViewPort.Y + 270) };
	_build[190].tex = RM_TEXTURE->getResource("Resource/Maptool/maptoolui/3.png");
	SPRITEMANAGER->renderRectTexture(_build[190].tex, &_build[190].rc1, &_build[190].rc2, 0, 0, 64, 64, rightViewPort.X + (DWORD)180, rightViewPort.Y + (DWORD)230);

	_build[191].rc2 = { (LONG)(rightViewPort.X + 230),(LONG)(rightViewPort.Y + 230),(LONG)(rightViewPort.X + 270),(LONG)(rightViewPort.Y + 270) };
	_build[191].tex = RM_TEXTURE->getResource("Resource/Maptool/maptoolui/4.png");
	SPRITEMANAGER->renderRectTexture(_build[191].tex, &_build[191].rc1, &_build[191].rc2, 0, 0, 64, 64, rightViewPort.X + (DWORD)230, rightViewPort.Y + (DWORD)230);
...
void objectClass::monster(int number)
{
	if (number != 0)
	{
		_monsterObject[number - 1]->render();
	}
}
