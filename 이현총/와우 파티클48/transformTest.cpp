...
HRESULT transformTest::init(void)
{
	//큐브생성
	this->CreateCube();
	_cubeTrans.SetWorldPosition(0, 0, 0);
...
void transformTest::release(void)
{
	SAFE_RELEASE(_vb);
	SAFE_RELEASE(_ib);
}

void transformTest::update(void)
{
...
void transformTest::render(void)
{
	//큐브 월드행렬 세팅
	...
	_cubeTrans.RenderGimozo();
}

void transformTest::CreateCube(void)
{
	//정점 버퍼 만들기
