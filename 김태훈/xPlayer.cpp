...
HRESULT xPlayer::init()
{
	...
	transform* playerTrans = _playerObject->_transform;
	...
	_attackBound.setBound(&D3DXVECTOR3(pos.x, pos.y + 0.5f, pos.z + 0.8), &D3DXVECTOR3(0.5, 0.5, 0.5));
...
{
	//if (_state == P_MOUNT || _state == P_ATTACK || _state == P_DEATH || _state == P_CASTSPELL)return;

	if (_state == P_MOUNT || _state == P_DEATH || _state == P_STUN || _state == P_DAMAGED) { return; }

	if (KEYMANAGER->isOnceKeyDown(VK_LBUTTON))
	{
		_state = P_ATTACK;
	}
	//LPCSTR타입으로 반환하는 GetName함수는 string으로 캐스팅이 필요합니다.
	string aa = _playerObject->_skinnedAnim->getNowPlayanim()->GetName();
