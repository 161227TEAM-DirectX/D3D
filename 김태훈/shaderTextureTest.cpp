#include "stdafx.h"
#include "shaderTextureTest.h"


HRESULT shaderTextureTest::init(void)
{
	//메인카메라
...
void shaderTextureTest::release(void)
{
	SAFE_RELEASE(_vb);
	SAFE_RELEASE(_ib);
...
void shaderTextureTest::update(void)
{
...
void shaderTextureTest::render(void)
{
	//이펙트에 전역 변수값을 세팅한다.
	...
	//타일수 세팅
	_effect->SetFloat("tilingX", 2);
	_effect->SetFloat("tilingY", 2);
...
void shaderTextureTest::CreateQuad()
{
	//정점 버퍼 생성
