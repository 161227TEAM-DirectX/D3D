...
float4 ps_main( VS_OUTPUT In ) : COLOR0	
{
	...
	//섞인 컬러...
	float3 mixColor = grayColor * vColor.rgb;
	
	//원본 컬러
	float3 baseColor = texColor.rgb;

	//마스크샘플링 ( r 값만 뺀다 )
	float mark = tex2D( MyMaskSamp, uv ).r;
	
	//float3 finalColor = lerp( baseColor, vColor.rgb, mark );


	//원본컬러 섞이는 부분 회색처리
	baseColor = lerp( baseColor, grayColor, saturate( mark * 5.0f) );

	float3 finalColor = baseColor +  mixColor * mark;
